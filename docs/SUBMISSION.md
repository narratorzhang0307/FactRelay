...
Fact Atlas 是一个基于 Gonka 的可验证个人知识地图：Signals 主 Agent 按主题和日期组织全球新闻，FactRelay 主 Agent 编排六个核验子 Agent 与 Skills，只把有证据与回执的知识落位到私人星球。

### English

Fact Atlas is a verifiable personal knowledge map on Gonka. A Signals supervisor organizes global news by topic and date, while a FactRelay supervisor coordinates six bounded verification subagents before a human chooses what enters the private Atlas.
...
Fact Atlas 提供两条知识路径：Relay 让用户主动提交书籍、电影、聊天、新闻或社交媒体中的主张；Signals 主 Agent 编排 AI、科技、金融、气候能源、科学、健康、城市文化与公共政策八个主题子 Agent 和新闻检索 Skills。用户可以选择主题和日期，一次左右滑动浏览一张候选新闻卡；每张卡都可进入核验，而不会被无限信息流淹没。Gonka 完成第一道重要性筛选，但该分数不是真实度判断。

用户选中候选主张后，FactRelay 主 Agent 将核验拆成受理、证据、调查、质疑、裁决和回执六个边界清晰的子 Agent，并调用输入校验、SSRF 防护、来源检索、JSON 规范化、证据评分和回执保全等可复用 Skills。Kimi-K2.6 担任调查方，MiniMax-M2.7 以质疑方角色检查循环引用、时间错位、因果跳跃和遗漏背景。最终 Truth Score 由确定性代码计算，并与来源账本、模型分歧和原始 Gonka Request ID 一起展示。最后，地点必须由用户确认，不能确定的内容保留在未落位轨道。
...
Signals uses a supervisor, eight topic subagents, and reusable retrieval Skills to organize global public news by topic and date. It shows one swipeable candidate card at a time instead of an endless feed. FactRelay turns a selected card—or user-supplied text, public URL, or screenshot—into a traceable investigation. Its supervisor coordinates six bounded subagents for intake, evidence, investigation, challenge, deterministic judgment, and receipt preservation. Kimi-K2.6 investigates while MiniMax-M2.7 challenges source laundering, chronology errors, causal leaps, and omitted context. The interface keeps the evidence ledger, disagreement, execution path, and untouched upstream Gonka Request IDs visible.
...
React client
  ...
  → Signals supervisor: topic/date selection + one swipeable card
  → eight topic subagents + reusable public-news retrieval Skills
  ...
  → FactRelay supervisor + six bounded subagents + evidence ledger + provenance trace
...
| 00:15–00:35 | Signals topic/date card deck | “Signals 主 Agent 编排八个主题子 Agent；用户按主题和日期一次浏览一张候选卡。” | Signals organizes eight topic agents by date, one swipeable candidate card at a time. |
...
| 01:16–01:42 | FactRelay agent orchestration | “FactRelay 主 Agent 编排受理、证据、调查、质疑、裁决和回执六个子 Agent，并保留模型分歧。” | The FactRelay supervisor coordinates six bounded subagents and preserves disagreement. |
...
- [x] A Signals supervisor coordinates eight topic subagents, date selection, reusable retrieval Skills, and one swipeable candidate card at a time.
- [x] A FactRelay supervisor coordinates six bounded subagents for intake, evidence, investigation, challenge, deterministic judgment, and receipt preservation.
