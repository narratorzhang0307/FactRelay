...
| **Signals · 每日发现** | Eight topic agents scan public news and use Gonka to rank importance—never truth. / 八个主题 Agent 扫描公开新闻，用 Gonka 排序重要性，但不冒充真实度。 |
...
`npm run verify` runs strict TypeScript checking, **34 unit tests across 9 files**, and the production build. `npm audit --audit-level=low` currently reports zero known vulnerabilities.
...
| `GET` | `/api/signals?topic=...` | Public-news candidates ranked by Kimi through GonkaRouter |
